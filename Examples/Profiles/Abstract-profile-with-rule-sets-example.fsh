// @Name: Abstract Profile with Rule Sets Example
// @Description: This is an alternative approach to using abstract profiles to define constraints that are common to multiple profiles using a FHIR Shorthand feature called "Rule Sets" (https://build.fhir.org/ig/HL7/fhir-shorthand/reference.html#defining-rule-sets). See "Abstract Profile Example" for more information on abstract profiles.
...
1) Using an abstract profile will create a StructureDefinition resource for the abstract profile that will appear as part of the built Implementation Guide. A Rule Set in FHIR Shorthand is applied upstream of the IG build process by SUSHI, so it will **not** appear anywhere in the built IG; instead, in the built IG the common constraints will appear in the "child" profiles as if they were directly defined there, and the "child" profiles will inherit from the base resource (or another profile) rather than from a common abstract profile. (Note that "child" is in quotes here because the Rule Set does not actually establish an inheritance relationship.)
...
3) Profiles can have only a single parent, but multiple Rule Sets can be inserted into a profile. The latter effectively allows for multiple inheritance, but profilers should be aware that this creates complexity which may make FSH hard to read or introduce conflicting constraints.
*/
