...
// The contents of `^short` appears in the \"Description & Constraints\" column in the differential or snapshot tables of a built Implementation Guide.
// In this case, `. ^short` sets this for the root level of the element hierarchy displayed in the table. `someElement ^short` will set this text for a specific
// element, as is seen below.
* . ^short = "A short description of the profile"

// This text displays when clicking on an element's name in the differential or snapshot tables when building a built Implementation Guide.
// This can also be accessed in the "Detailed Descriptions" tab.
